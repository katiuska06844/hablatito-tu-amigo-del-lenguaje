...
type Place = { name: string; emoji: string; desc: string };

const placesByLevel: Record<DifficultyLevel, Place[]> = {
  1: [
    { name: "Casa", emoji: "🏡", desc: "Donde vivimos" },
    { name: "Campo", emoji: "🌾", desc: "Donde sembramos" },
    { name: "Río", emoji: "🏞️", desc: "Agua que corre" },
  ],
  2: [
    { name: "Casa", emoji: "🏡", desc: "Donde vivimos" },
    { name: "Campo", emoji: "🌾", desc: "Donde sembramos" },
    { name: "Mercado", emoji: "🏪", desc: "Donde compramos" },
    { name: "Escuela", emoji: "🏫", desc: "Donde aprendemos" },
    { name: "Río", emoji: "🏞️", desc: "Agua que corre" },
  ],
  3: [
    { name: "Casa", emoji: "🏡", desc: "Donde vivimos" },
    ...
    { name: "Mercado", emoji: "🏪", desc: "Donde compramos" },
    { name: "Escuela", emoji: "🏫", desc: "Donde aprendemos" },
    { name: "Montaña", emoji: "🏔️", desc: "Muy alto en los Andes" },
    { name: "Plaza", emoji: "⛲", desc: "Donde jugamos" },
  ],
};

const RuralActivity = ({ onBack, onPoints }: Props) => {
  const [level, setLevel] = useState<DifficultyLevel>(() => getLevel("rural"));
  const [items] = useState(() => placesByLevel[getLevel("rural")]);
  ...
  const handleVoiceMatch = useCallback(() => {
    setScore(s => s + 5);
    recordResult("rural", true);
    setLevel(getLevel("rural"));
    setTimeout(() => {
  ...
  if (done) return (
    <div className="app-shell andean-bg flex flex-col items-center justify-center min-h-dvh px-6 gap-6">
      <div className="animate-bounce-in text-center">
        ...
        <h2 className="text-3xl font-black text-foreground">¡Bravo!</h2>
        <p className="text-xl text-muted-foreground font-bold mt-2">Exploraste {items.length} lugares</p>
  ...
  return (
    <div className="app-shell andean-bg flex flex-col min-h-dvh px-5 py-6">
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-4" key={idx}>
        {!tapped && <p className="text-muted-foreground font-semibold">¡Toca para explorar!</p>}
